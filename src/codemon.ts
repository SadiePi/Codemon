...
export class Codemon extends Contestant<PBS> {
  ...
  public getSpecies(includeMutations = true): Species {
    return includeMutations && this.mutations
      ? { ...this.species, ...this.mutations }
      : this.species;
  }
  ...
  public mutate(mutations: Partial<Species>) {
    this.mutations = {
      name: `Mutated ${this.species.name}`,
      ...this.mutations,
      ...mutations,
    };
  }
  ...
  constructor(
    ...
    this.gender = decide(options.gender, context) ??
      decide(this.getSpecies().genders, this);
    this.originalAbility =
      this.ability =
        decide(options.ability, context) ??
          decide(choose(this.getSpecies().abilities.normal.length), null);
    this._originalNature = this._nature = decide(options.nature, context) ??
      getRandomNature(options);
    this.trainer = decide(options.trainer, context) ??
      { traditionalStrategy: config.wild };
  ...
  private resolveAbility(selector: AbilitySelector): Ability {
    if (selector === "hidden") {
      return this.getSpecies().abilities.hidden ??
        this.getSpecies().abilities.normal[0];
    }
    if (typeof selector === "number") {
      return this.getSpecies().abilities
        .normal[selector % this.getSpecies().abilities.normal.length];
    }
    return selector;
  }
  ...
  public set ability(ability: AbilitySelector) {
    ...
    this._abilityEntry = new StatusEntry<{ self: Codemon }>(this.ability, {
      self: this,
    });
  }
  ...
  public get gender() {
    return this.getSpecies().overrideGender?.(this, this._gender) ??
      this._gender;
  }
  ...
  //   const action = decide(this.trainer.traditionalStrategy.chooseAction, {
  //     combatant: this,
  //     battle,
  //   });
  //   const choice = battle.getTargetChoice(action, this);
  //   const targets = decide(this.trainer.traditionalStrategy.chooseTarget, {
  //     action,
  //     combatant: this,
  //     choice,
  //     battle,
  ...
  // public receiveTraditionalTargetEffects(
  //   effects: TargetEffects<T>,
  ...
  //   if (effects.attack) {
  //     receipt.attack = this.receiveTraditionalAttack(effects.attack, context);
  //   }
  //   if (effects.hp) receipt.hp = this.receiveTraditionalHp(effects.hp, context);
  //   if (effects.status) {
  //     receipt.status = this.receiveTraditionalStatus(effects.status, context);
  //   }
  //   if (effects.stages) {
  //     receipt.stages = this.receiveTraditionalStages(effects.stages, context);
  //   }
  //   if (effects.faint) {
  //     receipt.faint = this.receiveTraditionalFaint(effects.faint, context);
  //   }
  //   if (effects.ball) {
  //     receipt.ball = this.receiveTraditionalBall(effects.ball, context);
  //   }
  //   if (effects.eject) {
  //     receipt.eject = this.receiveTraditionalEject(effects.eject, context);
  //   }
  ...
  //     config.codemon.limitDamageToRemainingHP
  //       ? Math.min(product, this.stats.hp.current)
  //       : product,
  //   );

  //   this.stats.hp.current -= total;
  //   messages.push(
  //     ...decide(config.locale.codemon.traditional.attack.damage, {
  //       context,
  //       total,
  //     }),
  //   );
  ...
  //   effect: Decider<
  //     SingleOrArray<StatusEffect<T>> | undefined,
  //     TargetContext<T>
  //   >,
  //   context: TargetContext<T>,
  // ): StatusReceipt {
  ...
  //     messages.push(
  //       ...decide(config.locale.codemon.traditional.status.apply, {
  //         context,
  //         entry,
  //       }),
  //     );
  ...
  //     0,
  //     Math.min(this.stats.hp.max, this.stats.hp.current + hp),
  //   );
  //   const difference = this.stats.hp.current - old;

  //   const messages = [
  //     ...decide(config.locale.codemon.traditional.hp, { context, difference }),
  //   ];
  ...
  //     if (stages[stat]) this.stats[stat].stage.modify(stages[stat]!);
  //   }

  //   const messages = [
  //     ...decide(config.locale.codemon.traditional.stages, { context, stages }),
  //   ];
  ...
  //   return {
  //     success: true,
  //     messages: [fmt.red(`${this.name} fainted!`)],
  //     actual: true,
  //   };
  // }
  ...
  //   const bonusLevel = this.stats.level < 13
  //     ? Math.max((36 - 2 * this.stats.level) / 10, 1)
  //     : 1;
  ...
  //   const a =
  //     Math.floor(
  //       (3 * maxHP - 2 * currentHP) * 4096 * darkGrass * rateModified *
  //         ballBonus * badgePenalty,
  //     ) *
  //     bonusLevel *
  //     bonusStatus *
  //     bonusMisc;
  ...
  //       actual: ballBonus,
  //       messages: [
  //         ...decide(config.locale.codemon.traditional.ball.immediate, {
  //           context,
  //           a,
  //         }),
  //       ],
  ...
  //   for (
  //     let check = 0; check < config.battle.traditional.shakeChecks; check++
  //   ) {
  ...
  //         actual: ballBonus,
  //         messages: [
  //           ...decide(config.locale.codemon.traditional.ball.escape, {
  //             context,
  //             a,
  //             b,
  //             check,
  ...
  //     actual: ballBonus,
  //     messages: [
  //       ...decide(config.locale.codemon.traditional.ball.caught, {
  //         context,
  //         a,
  //         b,
  //       }),
  ...
  //     success: true,
  //     messages: [
  //       ...decide(config.locale.codemon.traditional.reward, {
  //         context,
  //         reward,
  //       }),
  //     ],
  ...
  //     success: true,
  //     messages: [
  //       ...decide(config.locale.codemon.traditional.eject, { context }),
  //     ],
  //     actual: true,
  //   };
  // }
  ...
  //     success: true,
  //     messages: [
  //       ...decide(config.locale.codemon.traditional.disable, {
  //         context,
  //         move: disable,
  //       }),
  //     ],
  ...
  public getDesiredEvolutions(
    reasons?: Partial<ExternalEvoReasons>,
  ): Evolution[] {
  ...
  public override toString(short = false) {
    if (short) {
      return this.name + ` (${this.stats.hp.current}/${this.stats.hp.value()})`;
    }

    const identity = `${
      this.name !== this.getSpecies().name ? `${this.name}: ` : ""
    }${this.nature.name}, ${this.gender.name} ${this.getSpecies().name}${
      this.name === this.getSpecies().name ? "" : " named " + this.name
    }`;

    const stats = this.stats.toString().replace("\n", "\n\t");

    const moves = this.moves.entries.map((m, i) => i + 1 + ". " + m.toString())
      .join("\n");
